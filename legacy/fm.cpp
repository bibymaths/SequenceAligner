...
class FMIndexSdsl {
 public:
  using csa_sdsl_type =
      sdsl::csa_wt<sdsl::wt_huff<sdsl::rrr_vector<63>>, 32, 32,
  ...
  FMIndexSdsl(const std::string& text, char sentinel = '$')
    ...
    std::string temp_file_name =
        "temp_sdsl_input_text_" + std::to_string(std::time(nullptr)) + ".dat";
    {
      std::ofstream temp_out_file(temp_file_name, std::ios::binary);
      if (!temp_out_file) {
        throw std::runtime_error(
            "Failed to create temporary file for SDSL construction: " +
            temp_file_name);
      }
      temp_out_file.write(text_with_sentinel.data(),
                          text_with_sentinel.length());
    }
    ...
    try {
      ...
      throw std::runtime_error(
          std::string("SDSL FM-Index construction failed: ") + e.what());
    }
    std::remove(temp_file_name.c_str());
    initialized = true;
  }
  ...
  std::pair<size_t, size_t> backward_search(const std::string& pattern) const {
    if (!initialized)
      throw std::runtime_error("FMIndexSdsl not initialized for search.");
    ...
    for (auto it = pattern.rbegin(); it != pattern.rend(); ++it) {
      ...
      // A robust check would be to see if original_char maps to a comp_char <
      // fm_index.sigma and if that comp_char was actually present. However, if
      // C[comp_char] + rank(r, comp_char) <= C[comp_char] + rank(l, comp_char),
      // range is empty.

      size_t new_l =
          fm_index.C[compressed_char] + fm_index.bwt.rank(l, compressed_char);
      size_t new_r =
          fm_index.C[compressed_char] + fm_index.bwt.rank(r, compressed_char);
  ...
  size_t count_occurrences(const std::string& pattern) const {
    if (!initialized)
      throw std::runtime_error("FMIndexSdsl not initialized for count.");
  ...
  sdsl::int_vector<64> locate_occurrences(const std::string& pattern) const {
    if (!initialized)
      throw std::runtime_error("FMIndexSdsl not initialized for locate.");
    return sdsl::locate(fm_index, pattern.begin(), pattern.end());
  }

  bool save(const std::string& filepath) const {
    if (!initialized) {
      std::cerr << "Warning: Attempting to save an uninitialized FMIndexSdsl."
                << std::endl;
      return false;
    }
    try {
      ...
      std::cerr << "Error saving FMIndex to " << filepath << ": " << e.what()
                << std::endl;
      return false;
    }
  }
  ...
  bool load(const std::string& filepath) {
    try {
      ...
      std::cerr << "Error loading FMIndex from " << filepath << ": " << e.what()
                << std::endl;
      initialized = false;
      return false;
    }
  ...
  size_t get_alphabet_size() const {
    if (!initialized) return 0;
    return fm_index
        .sigma;  // sigma is the size of the effective alphabet [cite: 33]
  }
  ...
  void print_c_table_like_info(std::ostream& os) const {
    ...
    // fm_index.comp2char maps compressed codes to original characters [cite:
    // 33] fm_index.C is indexed by compressed character codes [cite: 33]
    for (size_t i = 0; i < fm_index.sigma;
         ++i) {  // i is the compressed char code
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 2; i < argc; ++i) {
    ...
    if (arg == "-s" && i + 1 < argc) {
      ...
      std::cerr << "Warning: Unknown argument or missing value for -s: '" << arg
                << "'" << std::endl;
    }
  }
  ...
  auto process_sequence = [&](const std::string& header,
                              const std::string& sequence_data) {
    if (sequence_data.empty()) {
      std::cerr << "[" << header << "] Skipping empty sequence." << std::endl;
      return;
    }
    std::cerr << "Processing sequence: " << header << " (length "
              << sequence_data.length() << ")" << std::endl;

    std::string sanitized_header = header;
    std::replace_if(
        sanitized_header.begin(), sanitized_header.end(),
    ...
    try {
      ...
      if (!idx.save(fname)) {
        ...
        std::cerr << "[" << header
                  << "] Text length (incl. sentinel)=" << idx.get_text_length();
        ...
        if (idx.get_text_length() > 10) {
          ...
          if (idx.get_text_length() > 50) {
            test_pattern = sequence_data.substr(
                std::min((size_t)20, sequence_data.length() - 1),
                std::min((size_t)3,
                         sequence_data.length() -
                             std::min((size_t)20, sequence_data.length() - 1)));
          } else {
            test_pattern = sequence_data.substr(
                0, std::min((size_t)3, sequence_data.length()));
          }

          if (!test_pattern.empty()) {
            std::cerr << "  Testing with pattern: \"" << test_pattern << "\""
                      << std::endl;
            auto   sa_range  = idx.backward_search(test_pattern);
            size_t count_val = idx.count_occurrences(
                test_pattern);  // Renamed from count to avoid conflict
            std::cerr << "    Pattern '" << test_pattern << "': SA range ["
                      << sa_range.first << ", " << sa_range.second
                      << "), Count (sdsl::count): " << count_val << std::endl;
            ...
            if (count_val > 0 && count_val < 15) {
              ...
              std::cerr << "    Pattern occurs " << count_val
                        << " times (locations not printed)." << std::endl;
            }
          }
        }
      }
    } catch (const std::exception& e) {
      std::cerr << "ERROR processing sequence " << header << ": " << e.what()
                << std::endl;
    }
  };
