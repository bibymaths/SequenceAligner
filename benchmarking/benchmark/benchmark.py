...
def aggregate_metrics(
    run_results: List[Dict[str, Any]], metric_key: str
    ...
    for run_idx in range(1, runs + 1):
        ...
        for seq_type, cfg_section in [("dna", dna_cfg), ("protein", protein_cfg)]:
            ...
            if not query_path or not target_path:
                logging.warning(
                    f"No {seq_type} query/target specified; skipping {seq_type} runs"
                )
                continue
            ...
            for tool_name in tool_map[seq_type]:
                ...
                log_file = os.path.join(
                    logs_dir, f"{tool_name}_{seq_type}_run{run_idx}.log"
                )
                res = runner(
                ...
                if res is None:
                    ...
                    continue
                # Append results
                all_results.setdefault(seq_type, {}).setdefault(tool_name, []).append(
                    res
                )
    ...
    for seq_type, tool_dict in all_results.items():
        for tool_name, run_results in tool_dict.items():
            ...
            runtimes = [
                ...
                if isinstance(r.get("runtime"), (int, float))
            ]
            mems = [
                r["memory"]
                for r in run_results
                if isinstance(r.get("memory"), (int, float))
            ]
            rt_stats = (
                utils.aggregate_numbers(runtimes)
                if runtimes
                else {k: None for k in ["mean", "median", "std", "min", "max"]}
            )
            mem_stats = (
                utils.aggregate_numbers(mems)
                if mems
                else {k: None for k in ["mean", "median", "std", "min", "max"]}
            )
