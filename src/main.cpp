...
double GAP_EXTEND =
    -1.0;  // penalty to extend an existing gap in global alignment
...
inline int blosum62_score(char x, char y) {
  ...
  if (ix == 255 || iy == 255)
    throw std::runtime_error(std::string("Invalid protein code: ") + x + "," +
                             y);
...
void showProgressBar(int progress, int total) {
  ...
  auto format_hms = [](long secs) {
    ...
    if (h) os << h << ":";
    os << std::setw(2) << std::setfill('0') << m << ":" << std::setw(2)
       << std::setfill('0') << s;
    return os.str();
  };
  ...
  std::cout << "] " << std::setw(3) << int(ratio * 100) << "% " << progress
            << "/" << total << " Elapsed: " << format_hms(elapsed)
            << " ETA: " << format_hms(eta) << std::flush;
}
 ...
 *
 * For DNA headers, looks for the first pair of parentheses “(GENE)” and returns
 * GENE. For protein headers, takes the part after the second ‘|’ up to the
 * first underscore.
 *
...
std::string getGeneSymbol(const std::string& header, ScoreMode mode) {
  if (mode == MODE_DNA) {
    ...
    size_t close = (open != std::string::npos) ? header.find(')', open + 1)
                                               : std::string::npos;
 ...
 */
void savePlainAlignment(const std::string& header1, const std::string& header2,
                        const std::string& aligned1,
                        const std::string& aligned2, std::ostream& os) {
  // Write seq1
  os << '>' << header1 << '\n'
     << aligned1
     << '\n'
 ...
 * @param os       Output stream (e.g. ofstream).
 */
void saveLCS(const std::string& id, const std::string& lcs_str,
             std::ostream& os) {
 ...
 */
void initAffineDP(int n, vector<int>& prev_row, vector<int>& prev_gapX,
                  vector<int>& prev_gapY, bool isGlobal) {
 ...
 */
void computeAffineDPRow(
    int i, const string& x, const string& y,
    ...
    ScoreFn       score_fn) {
  // (void)prev_gapY; // prev_gapY is actually used for curr_gapY calculation if
  // it's E_{i-1}
  int n = y.size();
  ...
  // This depends on global F_{i,0} which is S_{i-1,0} + GAP_OPEN or F_{i-1,0} +
  // GAP_EXTEND
  if (i > 0) {  // Only for rows i > 0
    ...
    if (curr_gapX[0] == f_open &&
        curr_gapX[0] >= f_extend) {  // Prefer opening if scores are equal
      curr_trace_row[0] =
          'F';  // Indicates F state, from S_{i-1,0} (vertical move)
    } else {
      curr_trace_row[0] =
          'f';  // Indicates F state, from F_{i-1,0} (vertical extend)
    }
    // E_{i,0} is usually -infinity for global alignment as you can't have a gap
    // in X before Y starts.
  ...
  for (int j = 1; j <= n; ++j) {
    ...
    int e_open_score =
        curr_row[j - 1] + GAP_OPEN;  // curr_row[j-1] is S_{i,j-1}
    int e_extend_score =
        curr_gapY[j - 1] + GAP_EXTEND;  // curr_gapY[j-1] is E_{i,j-1}
    ...
    // score_fn(x[i-1],y[j-1])
    int mscore = score_fn(x[i - 1], y[j - 1]);
    int diag_predecessor_score =
        max({prev_row[j - 1], prev_gapY[j - 1],
             prev_gapX[j - 1]});  // Max of S, E, F from top-left
    int match_val = diag_predecessor_score + mscore;

    // Determine S_{i,j} and the pointer
    // Preference: Match > Gap in X (horizontal E) > Gap in Y (vertical F) -
    // adjust as needed
    if (match_val >= curr_gapY[j] && match_val >= curr_gapX[j]) {
      curr_row[j]       = match_val;
      curr_trace_row[j] = 'M';  // Match/Mismatch
    } else if (curr_gapY[j] >=
               curr_gapX[j]) {  // Prefer E over F if match_val is not the max
      curr_row[j] = curr_gapY[j];
      // Distinguish if E came from open or extend
      if (curr_gapY[j] == e_open_score &&
          curr_gapY[j] >= e_extend_score) {  // Prefer open
      ...
      if (curr_gapX[j] == f_open_score &&
          curr_gapX[j] >= f_extend_score) {  // Prefer open
...
/**
 * @brief Perform global sequence alignment using the Needleman-Wunsch
 * algorithm.
 *
 * This function computes the global alignment of two sequences using affine gap
 * scoring. It initializes the DP matrix, fills it row by row, and performs
 ...
 * @param score_fn  Scoring function for match/mismatch.
 */
void globalalign(const string& x, const string& y, const string& header1,
                 const string& header2, const std::string& outdir,
  ...
  vector<vector<char>> fullTraceMatrix(
      m + 1, vector<char>(n + 1));  // To store all pointers
  ...
  for (int j = 1; j <= n; ++j) {
    prev_gapY[j] =
        (j == 1)
            ? (prev_row[j - 1] + GAP_OPEN)
            : (prev_gapY[j - 1] + GAP_EXTEND);  // E_0,j from S_0,j-1 or E_0,j-1
                                                // S_0,j-1 is prev_row[j-1]
    prev_row[j]  = prev_gapY[j];                // S_0,j = E_0,j
    prev_gapX[j] = INT_MIN / 2;  // F_0,j is not typically possible
    fullTraceMatrix[0][j] =
        (j == 1 && prev_gapY[j] == prev_row[j - 1] + GAP_OPEN) ? 'E' : 'e';
  }
  ...
  for (int i = 1; i <= m; ++i) {
    ...
    computeAffineDPRow(i, x, y, prev_row, prev_gapX, prev_gapY, curr_row,
                       curr_gapX, curr_gapY,
    ...
    // No need to swap curr_trace_row as it's freshly computed each time based
    // on S,E,F of previous.

    if (verbose) {
      if (i % 100 == 0 || i == m) {  // Adjusted progress update frequency
        showProgressBar(i, m);
      }
    }
  }

  // Score is in the last element of the S matrix (which is now in prev_row
  // after the loop)
  ...
  // The path is usually traced from (m,n) back to (0,0) or where alignment
  // starts
  ...
  while (cur_i > 0 || cur_j > 0) {
    ...
    if (trace_char == 'M') {  // Match/Mismatch
      ...
      cur_i--;
      cur_j--;
    } else if (trace_char == 'F' ||
               trace_char == 'f') {  // Gap in Y (x_i vs '-'), came from F state
      alignedX += x[cur_i - 1];
      alignedY += '-';
      cur_i--;
    } else if (trace_char == 'E' ||
               trace_char == 'e') {  // Gap in X (y_j vs '-'), came from E state
      alignedX += '-';
      alignedY += y[cur_j - 1];
      cur_j--;
    } else {  // Boundary conditions for i=0 or j=0
      if (cur_i == 0 &&
          cur_j > 0) {  // Reached top row, only gaps in X possible
        alignedX += '-';
        alignedY += y[cur_j - 1];
        cur_j--;
      } else if (cur_j == 0 &&
                 cur_i > 0) {  // Reached first col, only gaps in Y possible
        alignedX += x[cur_i - 1];
        alignedY += '-';
        cur_i--;
      } else {
        // Should not happen if matrix is well-formed and traceback starts from
        // m,n Or if fullTraceMatrix[0][0] was 'S' and we hit it.
        if (cur_i == 0 && cur_j == 0 && fullTraceMatrix[0][0] == 'S') break;
        // cerr << "Traceback error: unexpected pointer " << trace_char << " at
        // " << cur_i << "," << cur_j << endl;
        break;
      }
    }
...

  {
    ...
    for (auto& p : global_path) pf << p.first << " " << p.second << "\n";
  }
  ...
  if (outfile) {
    savePlainAlignment(getAccession(header1, mode), getAccession(header2, mode),
                       alignedX, alignedY, outfile);
    outfile.close();
  } else {
...
// Struct to hold scores and pointers for a single DP cell in affine local
// alignment
struct AffineDPScores {
  ...
  char ptr = 'X';  // Pointer for S_val traceback: 'M', 'E'/'e' (open/extend E),
                   // 'F'/'f' (open/extend F), 'X' (stop/zero)
};

/**
 * @brief Compute a single cell in the local affine DP matrix.
 *
 * This function computes the scores for a single cell in the local alignment DP
 * matrix using affine gap penalties. It returns an AffineDPScores struct
 * containing the scores and the pointer for traceback.
 *
...
AffineDPScores compute_local_affine_cell(
  ...
  if (m_val >= current_E_val && m_val >= current_F_val) {
    ...
    if (current_E_val > 0) {
      cell_scores.ptr =
          (current_E_val == e_open_score && current_E_val >= e_extend_score)
              ? 'E'
              : 'e';
    } else {
      cell_scores.ptr = 'X';
    ...
    if (current_F_val > 0) {
      cell_scores.ptr =
          (current_F_val == f_open_score && current_F_val >= f_extend_score)
              ? 'F'
              : 'f';
    } else {
      cell_scores.ptr = 'X';
  ...
  // Final S score must also be >= 0 (implicitly handled if M,E,F are already
  // floored and S is max of them) If all M, E, F were 0, then S is 0.
  if (cell_scores.s_val == 0 &&
      (m_val > 0 || current_E_val > 0 || current_F_val > 0)) {
    // This case should not happen if M, E, F are already max(0, val) and S =
    // max(M,E,F) However, if S is strictly max(0, M, E, F) as one combined
    // step: cell_scores.s_val = std::max({0, m_val_unfloored,
    // current_E_val_unfloored, current_F_val_unfloored}); For clarity, ensure S
    // is max of already-floored M, E, F, and also >=0
  }
 ...
 *
 * This function computes the local alignment of two sequences using affine gap
 * scoring. It initializes the DP matrix, fills it row by row, and performs
 * traceback to get the aligned sequences.
 *
 * @param x         The first sequence (string).
...
void localalign(const std::string& x, const std::string& y,
  ...
  // For simplicity, let's assume each rank computes its S, E, F rows
  // independently for its block, receiving the necessary S, E, F values from
  // the previous rank's last computed row.

  int chunkSize = (m + num_procs - 1) / num_procs;
  int start_row_global =
      rank * chunkSize;  // 0-indexed global start row for this rank
  int end_row_global = std::min(start_row_global + chunkSize,
                                m);  // 0-indexed global end row (exclusive)
  ...
  std::vector<std::vector<int>> s_block(local_num_rows + 1,
                                        std::vector<int>(n + 1, 0));
  std::vector<std::vector<int>> e_block(local_num_rows + 1,
                                        std::vector<int>(n + 1, 0));
  std::vector<std::vector<int>> f_block(local_num_rows + 1,
                                        std::vector<int>(n + 1, 0));
  ...
  // Initialization for local alignment: S[0,j]=0, E[0,j]=0, F[0,j]=0. S[i,0]=0,
  // E[i,0]=0, F[i,0]=0. S[0,0]=0. E[0,0]=NEG_INF, F[0,0]=NEG_INF (conceptually
  // for transitions into first cell). We initialize S,E,F edges to 0. Let's
  // stick to that for simplicity.

  if (rank > 0) {
    // Receive S, E, F values of the last row from rank-1
    // These become the 'previous' row (index 0 in s_block, e_block, f_block)

    MPI_Recv(s_block[0].data(), n + 1, MPI_INT, rank - 1, 0, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
    MPI_Recv(e_block[0].data(), n + 1, MPI_INT, rank - 1, 1, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
    MPI_Recv(f_block[0].data(), n + 1, MPI_INT, rank - 1, 2, MPI_COMM_WORLD,
             MPI_STATUS_IGNORE);
  } else {
    // Rank 0 initializes its 'previous' row (s_block[0], e_block[0],
    // f_block[0]) to all zeros. s_block[0] is already all zeros by vector
    // initialization. e_block[0] and f_block[0] should also be all zeros for
    // local affine.
  }
  ...
  for (int ii = 1; ii <= local_num_rows; ++ii) {  // ii is 1-indexed local row
    int global_i_idx =
        start_row_global + ii - 1;  // 0-indexed global character from x
    ...
    for (int j = 1; j <= n; ++j) {  // j is 1-indexed column
      ...
      AffineDPScores cell = compute_local_affine_cell(
          s_block[ii - 1][j - 1], e_block[ii - 1][j - 1],
          f_block[ii - 1][j - 1],                  // S,E,F from diag_prev
      ...
      // Pointers not stored globally here, only during final traceback
      // recomputation

      if (cell.s_val > localBest.score) {
        localBest = {cell.s_val, global_i_idx + 1,
                     j};  // Store 1-indexed matrix cell coords
      }
    }
  ...
  if (rank + 1 < num_procs) {
    MPI_Send(s_block[local_num_rows].data(), n + 1, MPI_INT, rank + 1, 0,
             MPI_COMM_WORLD);
    MPI_Send(e_block[local_num_rows].data(), n + 1, MPI_INT, rank + 1, 1,
             MPI_COMM_WORLD);
    MPI_Send(f_block[local_num_rows].data(), n + 1, MPI_INT, rank + 1, 2,
             MPI_COMM_WORLD);
  }

  // Gather best hits (similar to before, localBest.i and localBest.j are
  // 1-indexed matrix cell coords)
  ...
  MPI_Gather(mine, 4, MPI_INT, rank == 0 ? all_best_hits_info.data() : nullptr,
             4, MPI_INT, 0, MPI_COMM_WORLD);

  int bestScore_global = 0, bestRank_global = 0, bestI_global = 0,
      bestJ_global = 0;  // 1-indexed cell coords
  if (rank == 0) {
    for (int r_idx = 0; r_idx < num_procs; ++r_idx) {
      ...
      if (s > bestScore_global) {
        ...
        bestI_global =
            all_best_hits_info[4 * r_idx + 2];  // This is 1-indexed global row
        bestJ_global =
            all_best_hits_info[4 * r_idx + 3];  // This is 1-indexed global col
      }
    }
  }
  ...
  if (rank == bestRank_global) {
    ...
    // Effectively, perform small_align_core logic here for the relevant
    // sub-sequences.

    int tb_m = bestI_global;  // Length of x subseq for traceback (1-indexed
                              // matrix size)
    int tb_n = bestJ_global;  // Length of y subseq for traceback

    if (tb_m > 0 &&
        tb_n > 0) {  // Only proceed if there's a region to traceback
      std::vector<std::vector<int>>  s_trace(tb_m + 1,
                                             std::vector<int>(tb_n + 1, 0));
      std::vector<std::vector<int>>  e_trace(tb_m + 1,
                                             std::vector<int>(tb_n + 1, 0));
      std::vector<std::vector<int>>  f_trace(tb_m + 1,
                                             std::vector<int>(tb_n + 1, 0));
      std::vector<std::vector<char>> p_trace(tb_m + 1,
                                             std::vector<char>(tb_n + 1, 'X'));

      // Initialize 0th row and col for S, E, F matrices to 0 for local affine

      // S[0][0] is 0. E[0][0], F[0][0] are effectively NEG_INF for transitions
      // but stored as 0.

      for (int i = 1; i <= tb_m; ++i) {
        for (int j = 1; j <= tb_n; ++j) {
          AffineDPScores cell = compute_local_affine_cell(
              s_trace[i - 1][j - 1], e_trace[i - 1][j - 1],
              f_trace[i - 1][j - 1], s_trace[i][j - 1], e_trace[i][j - 1],
              s_trace[i - 1][j], f_trace[i - 1][j], x[i - 1], y[j - 1],
              score_fn  // Use original x, y characters
          );
      ...
      while (cur_i > 0 || cur_j > 0) {
        if (s_trace[cur_i][cur_j] == 0 && p_trace[cur_i][cur_j] == 'X')
          break;  // End of local alignment
        if (p_trace[cur_i][cur_j] == 'X' && (cur_i > 0 || cur_j > 0))
          break;  // Safety break if X is hit prematurely
        ...
        if (ptr_char == 'M') {
          ...
          --cur_j;
                   ...
                   ptr_char == 'f') {  // Gap in Y (target), x[cur_i-1] vs '-'
          alignedX.push_back(x[cur_i - 1]);
          alignedY.push_back('-');
          --cur_i;
        } else if (ptr_char == 'E' ||
                   ptr_char == 'e') {  // Gap in X (query), y[cur_j-1] vs '-'
          alignedX.push_back('-');
          alignedY.push_back(y[cur_j - 1]);
          --cur_j;
        ...
        local_path.emplace_back(cur_j,
                                cur_i);  // Add current point after moving
      }
      std::reverse(alignedX.begin(), alignedX.end());
      std::reverse(alignedY.begin(), alignedY.end());
      std::reverse(local_path.begin(),
                   local_path.end());  // Path is built backwards
    }
  }
  ...
  std::vector<std::vector<int>>
      fullDP_S_affine;  // This will be the gathered S-matrix on Rank 0
  ...
  // Each rank (except 0) sends its s_block[1]...s_block[local_num_rows] data to
  // rank 0 Rank 0 receives them. This requires a more complex Gatherv or
  // Send/Recv loop. For simplicity, let's assume a Gatherv approach for the
  // S-matrix part:

  // 1. Each rank prepares its S-block data to send (local_num_rows * (n+1)
  // integers)
  ...
  if (local_num_rows > 0) {
    ...
    for (int i = 0; i < local_num_rows; ++i) {
      std::memcpy(&send_s_buffer[i * (n + 1)], s_block[i + 1].data(),
                  (n + 1) * sizeof(int));
    }
  }
  ...
  MPI_Gatherv(
      send_s_buffer.data(), send_s_buffer.size(), MPI_INT,
      ...
      rank == 0 ? s_displs.data() : nullptr, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    for (int r = 0; r < num_procs; ++r) {
      ...
      for (int i = 0; i < r_local_rows; ++i) {
        if (r_start_global_row + i < m) {
          ...
          std::memcpy(fullDP_S_affine[r_start_global_row + 1 + i]
                          .data(),  // Write to global_row + 1
  ...
  // Rank 0 handles output (after potentially receiving alignedX, alignedY from
  // bestRank_global)

  if (rank == 0) {
    if (bestRank_global != 0 &&
        bestScore_global > 0) { /* ... MPI_Recv alignedX, alignedY ... */
      int        len_ax, len_ay;
      MPI_Status status;
      MPI_Recv(&len_ax, 1, MPI_INT, bestRank_global, 10, MPI_COMM_WORLD,
               &status);
      alignedX.resize(len_ax);
      if (len_ax > 0)
        MPI_Recv(&alignedX[0], len_ax, MPI_CHAR, bestRank_global, 11,
                 MPI_COMM_WORLD, &status);
      MPI_Recv(&len_ay, 1, MPI_INT, bestRank_global, 12, MPI_COMM_WORLD,
               &status);
      alignedY.resize(len_ay);
      if (len_ay > 0)
        MPI_Recv(&alignedY[0], len_ay, MPI_CHAR, bestRank_global, 13,
                 MPI_COMM_WORLD, &status);
    }

    // Save local_path (which was populated on bestRank_global and sent to rank
    // 0 if needed, or just written by bestRank_global) For simplicity, let's
    // assume bestRank_global writes its own path file if it computed one. Or,
    // if rank 0 is always bestRank_global (if not using MPI effectively or
    // small data), it has local_path. If bestRank_global is not 0, local_path
    // needs to be sent to rank 0 or written by bestRank_global. Let's assume
    // rank 0 will write it if it has it (i.e., if rank 0 was bestRank_global)

    if (rank == bestRank_global && bestScore_global > 0 &&
        !local_path.empty()) {
      ...
      if (pf_local) {
        ...
        cerr << "Error: Cannot open " << outdir + "/local_path.txt"
             << " for writing.\n";
      }
    } else if (rank == 0 && bestRank_global != 0 && bestScore_global > 0) {
      // If rank 0 needs to write the path but wasn't the bestRank, it needs to
      // receive local_path This part is complex to add here; for now, assume
      // bestRank handles its path file. Or, only save path if rank 0 *is*
      // bestRank.
    }
  }
  ...
  if (rank == 0) {
    // (Broadcast alignedX and alignedY from bestRank_global to rank 0 if
    // needed, or have bestRank_global do the printing of alignment. For
    // simplicity, let's assume bestRank_global (if not 0) sends its alignedX,
    // alignedY to rank 0)
    ...
    if (bestRank_global != 0 && bestScore_global > 0) {
      ...
      if (len_ax > 0)
        MPI_Send(alignedX.data(), len_ax, MPI_CHAR, 0, 11, MPI_COMM_WORLD);
      MPI_Send(&len_ay, 1, MPI_INT, 0, 12, MPI_COMM_WORLD);
      if (len_ay > 0)
        MPI_Send(alignedY.data(), len_ay, MPI_CHAR, 0, 13, MPI_COMM_WORLD);
    }
  ...
  if (rank == 0) {
    ...
    if (total > 0) {  // Calculate stats only if alignment exists
      for (size_t k = 0; k < total; ++k) {
        if (alignedX[k] == '-' || alignedY[k] == '-')
          ++gaps;
        else if (alignedX[k] == alignedY[k])
          ++matches;
      }
    }
    double identity = (total > 0) ? (double(matches) / total) : 0.0;
    double coverage = (total > 0) ? (double(total - gaps) / total)
                                  : 0.0;  // This is coverage of aligned region
 ...
 *
 * This function computes the LCS using dynamic programming and saves the
 * results.
 *
 * @param x         The first sequence (string).
 * @param y         The second sequence (string).
 ...
 **/
void lcs(
    const string& x, const string& y, const string& header1,
    const string& header2, const std::string& outdir,
    ScoreMode mode) {  // ScoreMode might not be relevant for LCS scoring itself
  const int            m = x.size(), n = y.size();
  vector<vector<int>>  L(m + 1, vector<int>(n + 1, 0));  // DP table for lengths
  vector<vector<char>> b(m + 1,
                         vector<char>(n + 1, ' '));  // Traceback pointers
  vector<pair<int, int>>
      lcs_path_coords;  // To store (j,i) matrix coordinates of the LCS path
    ...
    for (int j = 1; j <= n; ++j) {
      if (x[i - 1] == y[j - 1]) {
        ...
        b[i][j] = 'D';  // Diagonal
                 ...
                 L[i][j - 1]) {  // Tie-breaking: prefer up over left
        L[i][j] = L[i - 1][j];
        b[i][j] = 'U';  // Up
      } else {
        L[i][j] = L[i][j - 1];
  ...
  while (cur_i > 0 && cur_j > 0) {
    lcs_path_coords.emplace_back(cur_j,
                                 cur_i);  // Store current cell coordinates
    if (b[cur_i][cur_j] == 'D') {
      lcs_str += x[cur_i - 1];
      alignedX += x[cur_i - 1];
      alignedY += y[cur_j - 1];
  ...
  if (!outfile_alignment) {
    cerr << "Error: Cannot open "
         << outdir + "/lcs_alignment.fasta for writing\n";
  } else {
    savePlainAlignment(getAccession(header1, mode) + "_LCS_aligned",
                       getAccession(header2, mode) + "_LCS_aligned", alignedX,
                       alignedY, outfile_alignment);
  ...
  // Save lcs_path_coords (matrix cell coordinates of the path)
  {
    ...
    if (!pf) {
      cerr << "Error: Cannot open " << outdir + "/lcs_path.txt"
           << " for writing.\n";
    } else {
 ...
 *
 * This function initializes MPI, parses command line arguments, reads input
 * files, and performs sequence alignments based on user choices.
 *
  ...
  try {
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      if (arg == "--query" && i + 1 < argc) {
        ...
        if (rank == 0) {
          std::cout
              << "Usage: ./aligner --query <file1> --target <file2> --choice "
                 "<1|2|3|4> [--mode dna|protein] [--outdir DIR] [--verbose]\n"
    ...
    if (file1.empty() || file2.empty() || choice == -1) {
      if (rank == 0)
        std::cerr
            << "Missing required arguments: --query, --target, --choice\n";
      MPI_Finalize();
      return 1;
    }
