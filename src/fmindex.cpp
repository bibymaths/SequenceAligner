...
std::vector<int> suffix_array(const std::string& s) {
  ...
  for (int i = 0; i < n; ++i) {
    rank[i] =
        static_cast<unsigned char>(s[i]);  // Initial ranks from char values
  }
...
class FMIndex {
 public:
  ...
  void build_occ_table() {
    // Occ[ch][i] = number of occurrences of ch in BWT[0:i-1] (prefix of length
    // i)
    this->Occ.clear();
    // Initialize zero-row for all characters present in C table (and thus in
    // BWT)
    for (const auto& pair : this->C) {
      this->Occ[pair.first].push_back(0);
    }
    // Also initialize for any character not in C yet (though unlikely if C is
    // from BWT)
...
bool read_fasta_sequence(std::istream& in, std::string& header,
                         std::string& sequence) {
  std::string line;
  header.clear();
  sequence.clear();
  ...
  if (header.empty() &&
      !in.eof()) {  // Found content before a header or malformed
    ...
    if (in.eof() &&
        !header.empty()) { /* Fine, last sequence had no seq lines */
    } else if (in.eof() && header.empty())
      return false;  // Truly end of file.
  ...
  while (std::getline(in, line)) {
    ...
    if (line[0] == '>') {
      ...
      // the first line read is already processed or it is the first line of the
      // new sequence For now, we'll assume `std::getline` followed by
      // `in.peek()` or careful `unget` A simpler strategy for this parser:
  ...
  while (std::getline(*input_stream, line)) {
    ...
    if (line[0] == '>') {
      if (in_sequence) {  // Process previous sequence
        ...
        std::cerr << "[" << current_header
                  << "] BWT length=" << idx.bwt.length() << "  C={";
        bool first_c = true;
  ...
  if (in_sequence && !current_header.empty()) {
    ...
    if (fasta_filepath != "-") {
      ...
      std::string base_name =
          std::filesystem::path(fasta_filepath).stem().string();
    ...
    std::cerr << "[" << current_header << "] BWT length=" << idx.bwt.length()
              << "  C={";
    bool first_c = true;
    for (const auto& pair : idx.C) {
      if (!first_c) std::cerr << ", ";
