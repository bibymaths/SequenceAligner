...
void computeAffineDPRow(int i_row, const std::string& x_str,
  ...
  if (isGlobal) {
    ...
    curr_trace_s_row[0] =
        (curr_f_row[0] == f_i0_open && curr_f_row[0] >= f_i0_extend) ? 'F'
                                                                     : 'f';
  } else {
    curr_s_row[0]       = 0;
    curr_e_row[0]       = 0;
  ...
  for (int j_col = 1; j_col <= n_len; ++j_col) {
    ...
    int match_pred_score =
        std::max(s_diag_prev, std::max(e_diag_prev, f_diag_prev));
    ...
    curr_s_row[j_col] =
        std::max(m_val, std::max(curr_e_row[j_col], curr_f_row[j_col]));
    ...
    if (curr_s_row[j_col] > 0 ||
      ...
      if (curr_s_row[j_col] == m_val)
        curr_trace_s_row[j_col] = 'M';
...
AlignmentResult perform_sw_in_window(const std::string& sub1,
                                     const std::string& sub2, ScoreFn sfn,
                                     double go, double ge, int q_off,
                                     int t_off) {
  AlignmentResult res;
  res.score = 0;
  ...
  for (int i = 1; i <= m_len; ++i) {
    for (int j = 1; j <= n_len; ++j) {
      int m_pred  = std::max(s_mat[i - 1][j - 1],
                             std::max(e_mat[i - 1][j - 1], f_mat[i - 1][j - 1]));
      ...
      s_mat[i][j] =
          std::max(0, std::max(m_v, std::max(e_mat[i][j], f_mat[i][j])));
  ...
  if (max_s_val > 0) {
    ...
    int         m_check =
        std::max(
            (ci > 0 && cj > 0 ? s_mat[ci - 1][cj - 1] : INT_MIN / 2),
            std::max(
...
AlignmentResult align_segment_globally(const std::string& seg1,
                                       const std::string& seg2, ScoreFn sfn,
                                       double go, double ge) {
  AlignmentResult res;
...
LcsSegmentResult compute_lcs_for_segment(const std::string& seg1,
  ...
  std::vector<std::vector<int>>  L_mat(m_len + 1,
                                       std::vector<int>(n_len + 1, 0));
  std::vector<std::vector<char>> B_mat(m_len + 1,
                                       std::vector<char>(n_len + 1, ' '));
  for (int i = 1; i <= m_len; ++i) {
    for (int j = 1; j <= n_len; ++j) {
...
static inline void get_row_partition(int total_rows, int world_size,
                                     int world_rank, int& start_row_0based,
                                     int& row_count) {
...
static std::vector<std::vector<int>> gather_score_matrix_rows(
    ...
    int total_rows_without_row0, int n_cols_minus1, bool is_global,
    MPI_Comm comm) {
  ...
  get_row_partition(total_rows_without_row0, world_size, world_rank, start_row,
                    local_rows);

  std::vector<int> sendbuf(local_rows * ncols, 0);
  for (int i = 0; i < local_rows; ++i) {
    std::memcpy(&sendbuf[i * ncols], local_block_with_boundary[i + 1].data(),
                ncols * sizeof(int));
  }

  std::vector<int> recvcounts, displs, recvbuf;
  if (world_rank == 0) {
    recvcounts.resize(world_size, 0);
    displs.resize(world_size, 0);
    int total_elems = 0;
    for (int r = 0; r < world_size; ++r) {
      int rs = 0, rr = 0;
      get_row_partition(total_rows_without_row0, world_size, r, rs, rr);
      recvcounts[r] = rr * ncols;
      displs[r]     = total_elems;
  ...
  if (world_rank == 0) {
    ...
    for (int r = 0; r < world_size; ++r) {
      ...
      get_row_partition(total_rows_without_row0, world_size, r, rs, rr);
      for (int i = 0; i < rr; ++i) {
        std::memcpy(full[rs + 1 + i].data(), &recvbuf[displs[r] + i * ncols],
                    ncols * sizeof(int));
      }
    }
  }
...
static void save_path_file(const std::string&                      filename,
...
static AlignmentResult traceback_global_from_full(
  ...
  prev_s.assign(n + 1, 0);
  prev_e.assign(n + 1, INT_MIN / 2);
  prev_f.assign(n + 1, INT_MIN / 2);
  trace[0][0] = 'S';
  for (int j = 1; j <= n; ++j) {
    prev_e[j] =
        (j == 1) ? (prev_s[j - 1] + GAP_OPEN) : (prev_e[j - 1] + GAP_EXTEND);
    prev_s[j]   = prev_e[j];
    prev_f[j]   = INT_MIN / 2;
  ...
  for (int i = 1; i <= m; ++i) {
    computeAffineDPRow(i, x, y, prev_s, prev_e, prev_f, curr_s, curr_e, curr_f,
                       curr_tr, score_fn, true);
...
static AlignmentResult traceback_local_from_full(
    const std::string& x, const std::string& y, ScoreFn score_fn, int best_i,
    int best_j, std::vector<std::pair<int, int>>& local_path_out) {
  const int m = static_cast<int>(x.size()), n = static_cast<int>(y.size());
  std::vector<int>  prev_s, prev_e, prev_f, curr_s, curr_e, curr_f;
  std::vector<char> curr_tr;
  ...
  AlignmentResult res;
  res.score      = S[best_i][best_j];
  int         ci = best_i, cj = best_j;
  std::string ax, ay;
  local_path_out.clear();
  ...
  std::reverse(ax.begin(), ax.end());
  std::reverse(ay.begin(), ay.end());
  std::reverse(local_path_out.begin(), local_path_out.end());
  res.aligned_seq1 = ax;
  res.aligned_seq2 = ay;
  return res;
}
...
static LcsSegmentResult traceback_lcs_from_full(
    ...
    const std::vector<std::vector<int>>& L, std::vector<std::vector<char>>& B,
  ...
  LcsSegmentResult res;
  res.lcs_length = L[m][n];
  int         ci = m, cj = n;
  std::string lcs_rev, ax_rev, ay_rev;
  path_out.clear();
  ...
  std::reverse(lcs_rev.begin(), lcs_rev.end());
  std::reverse(ax_rev.begin(), ax_rev.end());
  std::reverse(ay_rev.begin(), ay_rev.end());
  std::reverse(path_out.begin(), path_out.end());
  res.lcs_string  = lcs_rev;
  res.gapped_seq1 = ax_rev;
  res.gapped_seq2 = ay_rev;
  return res;
}
...
void globalalign(const std::string& x_orig, const std::string& y_orig,
  ...
  const int m    = static_cast<int>(x_orig.size()),
            n    = static_cast<int>(y_orig.size());
  ...
  if (world_rank == 0 && target_fm_idx != nullptr) {
    ...
    if (k_anchor > 0) {
      std::vector<Seed> raw_seeds =
          generate_raw_seeds(x_orig, *target_fm_idx, k_anchor, 0, 1);
      if (!raw_seeds.empty()) {
  ...
  int anchor_count = (world_rank == 0 && use_anchoring)
                         ? static_cast<int>(best_chain.seeds.size())
                         : 0;
  ...
  if (use_anchoring) {
    MPI_Bcast(best_chain.seeds.data(),
              anchor_count * static_cast<int>(sizeof(Seed)), MPI_BYTE, 0,
              MPI_COMM_WORLD);
  }
  ...
  if (use_anchoring) {
    if (world_rank == 0 && verbose)
      std::cout << "Global alignment: FM-index anchors found (" << anchor_count
                << "). Using anchored segmentation.\n";

    std::vector<std::pair<std::string, std::string>> segments;
    int                                              cx = 0, cy = 0;
    for (const auto& anc : best_chain.seeds) {
      segments.push_back({x_orig.substr(cx, anc.query_pos - cx),
                          y_orig.substr(cy, anc.target_pos - cy)});
      cx = anc.query_pos + anc.len;
      cy = anc.target_pos + anc.len;
    }
    ...
    for (size_t i = 0; i < segments.size(); ++i) {
      if (static_cast<int>(i % world_size) == world_rank) {
        seg_results[i] =
            align_segment_globally(segments[i].first, segments[i].second,
                                   score_fn_val, GAP_OPEN, GAP_EXTEND);
      }
    }
    ...
    if (world_rank == 0) {
      for (size_t i = 0; i < segments.size(); ++i) {
        ...
        if (owner != 0) {
          MPI_Status st;
          MPI_Recv(&seg_results[i].score, 1, MPI_INT, owner,
                   static_cast<int>(i) * 10 + 0, MPI_COMM_WORLD, &st);
          int len1 = 0, len2 = 0;
          MPI_Recv(&len1, 1, MPI_INT, owner, static_cast<int>(i) * 10 + 1,
                   MPI_COMM_WORLD, &st);
          seg_results[i].aligned_seq1.resize(len1);
          if (len1 > 0)
            MPI_Recv(&seg_results[i].aligned_seq1[0], len1, MPI_CHAR, owner,
                     static_cast<int>(i) * 10 + 2, MPI_COMM_WORLD, &st);
          MPI_Recv(&len2, 1, MPI_INT, owner, static_cast<int>(i) * 10 + 3,
                   MPI_COMM_WORLD, &st);
          seg_results[i].aligned_seq2.resize(len2);
          if (len2 > 0)
            MPI_Recv(&seg_results[i].aligned_seq2[0], len2, MPI_CHAR, owner,
                     static_cast<int>(i) * 10 + 4, MPI_COMM_WORLD, &st);
        }
      }

      for (size_t i = 0; i < best_chain.seeds.size(); ++i) {
        final_aln.aligned_seq1 += seg_results[i].aligned_seq1;
        final_aln.aligned_seq2 += seg_results[i].aligned_seq2;
        final_aln.score += seg_results[i].score;
        const auto& anc   = best_chain.seeds[i];
        std::string exact = x_orig.substr(anc.query_pos, anc.len);
        final_aln.aligned_seq1 += exact;
        final_aln.aligned_seq2 += exact;
        for (char c : exact) final_aln.score += score_fn_val(c, c);
      }
      final_aln.aligned_seq1 += seg_results.back().aligned_seq1;
      final_aln.aligned_seq2 += seg_results.back().aligned_seq2;
      final_aln.score += seg_results.back().score;
    } else {
      for (size_t i = 0; i < segments.size(); ++i) {
        if (static_cast<int>(i % world_size) == world_rank) {
          MPI_Send(&seg_results[i].score, 1, MPI_INT, 0,
                   static_cast<int>(i) * 10 + 0, MPI_COMM_WORLD);
          int len1 = static_cast<int>(seg_results[i].aligned_seq1.size()),
              len2 = static_cast<int>(seg_results[i].aligned_seq2.size());
          MPI_Send(&len1, 1, MPI_INT, 0, static_cast<int>(i) * 10 + 1,
                   MPI_COMM_WORLD);
          if (len1 > 0)
            MPI_Send(seg_results[i].aligned_seq1.data(), len1, MPI_CHAR, 0,
                     static_cast<int>(i) * 10 + 2, MPI_COMM_WORLD);
          MPI_Send(&len2, 1, MPI_INT, 0, static_cast<int>(i) * 10 + 3,
                   MPI_COMM_WORLD);
          if (len2 > 0)
            MPI_Send(seg_results[i].aligned_seq2.data(), len2, MPI_CHAR, 0,
                     static_cast<int>(i) * 10 + 4, MPI_COMM_WORLD);
        }
      }
    }
  } else {
    if (world_rank == 0 && verbose)
      std::cout << "Global alignment: FM-index anchoring unavailable/failed. "
                   "Falling back to MPI full DP.\n";
    int start_row = 0, local_rows = 0;
    get_row_partition(m, world_size, world_rank, start_row, local_rows);
    std::vector<std::vector<int>> s_block(local_rows + 1,
                                          std::vector<int>(n + 1, 0)),
        e_block(local_rows + 1, std::vector<int>(n + 1, 0)),
        f_block(local_rows + 1, std::vector<int>(n + 1, 0));
    ...
    if (world_rank == 0) {
      ...
      MPI_Recv(s_block[0].data(), n + 1, MPI_INT, world_rank - 1, 100,
               MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(e_block[0].data(), n + 1, MPI_INT, world_rank - 1, 101,
               MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(f_block[0].data(), n + 1, MPI_INT, world_rank - 1, 102,
               MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }

    std::vector<int>  curr_s, curr_e, curr_f;
    std::vector<char> curr_tr;
    for (int ii = 1; ii <= local_rows; ++ii) {
      int global_i = start_row + ii;
      computeAffineDPRow(global_i, x_orig, y_orig, s_block[ii - 1],
                         e_block[ii - 1], f_block[ii - 1], curr_s, curr_e,
                         curr_f, curr_tr, score_fn_val, true);
      s_block[ii] = curr_s;
      e_block[ii] = curr_e;
      f_block[ii] = curr_f;
      if (verbose && world_rank == 0 && (global_i % 100 == 0 || global_i == m))
        showProgressBar(global_i, m);
    }

    if (world_rank + 1 < world_size) {
      MPI_Send(s_block[local_rows].data(), n + 1, MPI_INT, world_rank + 1, 100,
               MPI_COMM_WORLD);
      MPI_Send(e_block[local_rows].data(), n + 1, MPI_INT, world_rank + 1, 101,
               MPI_COMM_WORLD);
      MPI_Send(f_block[local_rows].data(), n + 1, MPI_INT, world_rank + 1, 102,
               MPI_COMM_WORLD);
    }

    std::vector<std::vector<int>> fullS =
        gather_score_matrix_rows(s_block, m, n, true, MPI_COMM_WORLD);

    if (world_rank == 0) {
      if (binary)
        writeDPMatrix(fullS, outdir + "/global_dp_matrix.bin");
      else if (txt)
        writeRawDPMatrix(fullS, outdir + "/global_dp_matrix.txt");
      final_aln =
          traceback_global_from_full(x_orig, y_orig, score_fn_val, global_path);
    }
  }
  ...
  if (world_rank == 0) {
    if (use_anchoring) {
      if (txt || binary)
        std::cout << "\nNotice: DP Matrix skipped during anchored Global "
                     "Alignment.\n";
      int tr_cx = m, tr_cy = n;
    ...
    for (size_t i = 0; i < total; ++i) {
      if (final_aln.aligned_seq1[i] == '-' || final_aln.aligned_seq2[i] == '-')
        ++gaps;
      else if (final_aln.aligned_seq1[i] == final_aln.aligned_seq2[i])
        ++matches;
    }
    double identity = total > 0 ? static_cast<double>(matches) / total : 0.0;
    double coverage =
        total > 0 ? static_cast<double>(total - gaps) / total : 0.0;

    std::string acc1  = getAccession(header1, mode_val),
                acc2  = getAccession(header2, mode_val);
    std::string gene1 = getGeneSymbol(header1, mode_val),
                gene2 = getGeneSymbol(header2, mode_val);

    save_path_file(outdir + "/global_path.txt", global_path);

    if (verbose) {
      std::cout << "\n\nGlobal Alignment Score: " << final_aln.score << "\n";
      std::cout << "Matches: " << matches << " | Gaps: " << gaps
                << " | Total: " << total << "\n";
      std::cout << "Identity: " << identity * 100.0
    ...
    if (fa)
      savePlainAlignment(acc1, acc2, final_aln.aligned_seq1,
                         final_aln.aligned_seq2, fa);
...
void localalign(const std::string& x, const std::string& y,
  ...
  bool            use_fmindex = false;
  AlignmentResult best_seed_extend;
  best_seed_extend.score = 0;
  ...
  if (world_rank == 0 && tfm_idx != nullptr) {
    ...
    if (k > 0) {
      ...
      if (!all_seeds.empty()) {
        ...
        MPI_Bcast(all_seeds.data(),
                  total_seeds * static_cast<int>(sizeof(Seed)), MPI_BYTE, 0,
                  MPI_COMM_WORLD);

        int s_start = (total_seeds / world_size * world_rank) +
                      std::min(world_rank, total_seeds % world_size);
        int s_end = (total_seeds / world_size * (world_rank + 1)) +
                    std::min(world_rank + 1, total_seeds % world_size);
        AlignmentResult local_best;
        local_best.score = 0;

        for (int si = s_start; si < s_end; ++si) {
          const auto& seed  = all_seeds[si];
          int         win_q = std::max(100, seed.len * 3),
              win_t         = std::max(100, seed.len * 3);
          int qws           = std::max(0, seed.query_pos - win_q),
              qwe           = std::min(m, seed.query_pos + seed.len + win_q);
          int tws           = std::max(0, seed.target_pos - win_t),
              twe           = std::min(n, seed.target_pos + seed.len + win_t);
          AlignmentResult cand = perform_sw_in_window(
              x.substr(qws, qwe - qws), y.substr(tws, twe - tws), sfn_val,
              GAP_OPEN, GAP_EXTEND, qws, tws);
        ...
        struct {
          int score;
          int rank;
        } mine{local_best.score, world_rank}, best{0, 0};
        MPI_Allreduce(&mine, &best, 1, MPI_2INT, MPI_MAXLOC, MPI_COMM_WORLD);

        if (best.score > 0) {
          int best_rank = best.rank, len1 = 0, len2 = 0;
          if (world_rank == best_rank) {
            best_seed_extend       = local_best;
            best_seed_extend.score = best.score;
            len1 = static_cast<int>(local_best.aligned_seq1.size());
            len2 = static_cast<int>(local_best.aligned_seq2.size());
          }
          MPI_Bcast(&len1, 1, MPI_INT, best_rank, MPI_COMM_WORLD);
          MPI_Bcast(&len2, 1, MPI_INT, best_rank, MPI_COMM_WORLD);
          MPI_Bcast(&best_seed_extend.query_start_orig, 1, MPI_INT, best_rank,
                    MPI_COMM_WORLD);
          MPI_Bcast(&best_seed_extend.query_end_orig, 1, MPI_INT, best_rank,
                    MPI_COMM_WORLD);
          MPI_Bcast(&best_seed_extend.target_start_orig, 1, MPI_INT, best_rank,
                    MPI_COMM_WORLD);
          MPI_Bcast(&best_seed_extend.target_end_orig, 1, MPI_INT, best_rank,
                    MPI_COMM_WORLD);

          if (world_rank != best_rank) {
            best_seed_extend.aligned_seq1.resize(len1);
            best_seed_extend.aligned_seq2.resize(len2);
          }
          if (len1 > 0)
            MPI_Bcast(&best_seed_extend.aligned_seq1[0], len1, MPI_CHAR,
                      best_rank, MPI_COMM_WORLD);
          if (len2 > 0)
            MPI_Bcast(&best_seed_extend.aligned_seq2[0], len2, MPI_CHAR,
                      best_rank, MPI_COMM_WORLD);
        }
      } else {
        int zero = 0;
    ...
    if (total_seeds > 0) {
      ...
      MPI_Bcast(all_seeds.data(), total_seeds * static_cast<int>(sizeof(Seed)),
  ...
  if (use_fmindex && best_seed_extend.score > 0) {
    ...
    if (world_rank == 0 && verbose)
      std::cout << "Local alignment: FM-index anchoring unavailable/failed. "
                   "Fallback to MPI full DP.\n";
    int start_row = 0, local_rows = 0;
    get_row_partition(m, world_size, world_rank, start_row, local_rows);
    std::vector<std::vector<int>> s_block(local_rows + 1,
                                          std::vector<int>(n + 1, 0)),
    ...
    if (world_rank > 0) {
      MPI_Recv(s_block[0].data(), n + 1, MPI_INT, world_rank - 1, 200,
               MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(e_block[0].data(), n + 1, MPI_INT, world_rank - 1, 201,
               MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(f_block[0].data(), n + 1, MPI_INT, world_rank - 1, 202,
               MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    ...
    for (int ii = 1; ii <= local_rows; ++ii) {
      int global_i = start_row + ii;
      computeAffineDPRow(global_i, x, y, s_block[ii - 1], e_block[ii - 1],
                         f_block[ii - 1], curr_s, curr_e, curr_f, curr_tr,
                         sfn_val, false);
      s_block[ii] = curr_s;
      e_block[ii] = curr_e;
      f_block[ii] = curr_f;
      for (int j = 1; j <= n; ++j) {
        if (s_block[ii][j] > local_best_score) {
          local_best_score = s_block[ii][j];
          local_best_i     = global_i;
          local_best_j     = j;
        }
      }
      if (verbose && world_rank == 0 && (global_i % 100 == 0 || global_i == m))
    ...
    if (world_rank + 1 < world_size) {
      MPI_Send(s_block[local_rows].data(), n + 1, MPI_INT, world_rank + 1, 200,
               MPI_COMM_WORLD);
      MPI_Send(e_block[local_rows].data(), n + 1, MPI_INT, world_rank + 1, 201,
               MPI_COMM_WORLD);
      MPI_Send(f_block[local_rows].data(), n + 1, MPI_INT, world_rank + 1, 202,
               MPI_COMM_WORLD);
    }

    int mine[4] = {local_best_score, world_rank, local_best_i, local_best_j};
    std::vector<int> gathered;
    if (world_rank == 0) gathered.resize(4 * world_size, 0);
    MPI_Gather(mine, 4, MPI_INT, world_rank == 0 ? gathered.data() : nullptr, 4,
               MPI_INT, 0, MPI_COMM_WORLD);

    int best_score = 0, best_i = 0, best_j = 0;
    if (world_rank == 0) {
      for (int r = 0; r < world_size; ++r) {
        if (gathered[4 * r + 0] > best_score) {
          best_score = gathered[4 * r + 0];
          best_i     = gathered[4 * r + 2];
          best_j     = gathered[4 * r + 3];
        }
      }
    }
    ...
    std::vector<std::vector<int>> fullS =
        gather_score_matrix_rows(s_block, m, n, false, MPI_COMM_WORLD);
    if (world_rank == 0) {
      if (binary)
        writeDPMatrix(fullS, odir + "/local_dp_matrix.bin");
      else if (txt)
        writeRawDPMatrix(fullS, odir + "/local_dp_matrix.txt");
      final_aln =
          traceback_local_from_full(x, y, sfn_val, best_i, best_j, local_path);
  ...
  if (world_rank == 0) {
    if (use_fmindex) {
      ...
      int tr_cx = final_aln.query_end_orig + 1,
          tr_cy = final_aln.target_end_orig + 1;
    ...
    std::string gene1 = getGeneSymbol(h1, mval),
                gene2 = getGeneSymbol(h2, mval);
    ...
    if (verbose) {
      ...
      std::cout << "Time: " << final_aln.time_ms << " ms\n\n";
      if (final_aln.score > 0)
        printColoredAlignment(final_aln.aligned_seq1, final_aln.aligned_seq2);
    }

    std::ofstream fa(odir + "/local_alignment.fasta");
    if (fa)
      savePlainAlignment(acc1 + "_local", acc2 + "_local",
                         final_aln.aligned_seq1, final_aln.aligned_seq2, fa);
...
void lcs(const std::string& x_o, const std::string& y_o,
  ...
  if (world_rank == 0 && tfm_idx_lcs != nullptr) {
    ...
    if (k > 0) {
      std::vector<Seed> raw_seeds =
          generate_raw_seeds(x_o, *tfm_idx_lcs, k, 0, 1);
      if (!raw_seeds.empty()) {
        best_chain    = find_best_seed_chain(raw_seeds, 1);
        use_anchoring = !best_chain.seeds.empty();
      }
  ...
  if (use_anchoring) {
    ...
    int                                              cx = 0, cy = 0;
    for (const auto& anc : best_chain.seeds) {
      segments.push_back({x_o.substr(cx, anc.query_pos - cx),
                          y_o.substr(cy, anc.target_pos - cy)});
      cx = anc.query_pos + anc.len;
      cy = anc.target_pos + anc.len;
    }
    ...
    for (size_t i = 0; i < segments.size(); ++i) {
      if (static_cast<int>(i % world_size) == world_rank)
        seg_results[i] =
            compute_lcs_for_segment(segments[i].first, segments[i].second);
    }
    ...
    if (world_rank == 0) {
      for (size_t i = 0; i < segments.size(); ++i) {
        ...
        if (owner != 0) {
          MPI_Status st;
          MPI_Recv(&seg_results[i].lcs_length, 1, MPI_INT, owner,
                   static_cast<int>(i) * 10 + 0, MPI_COMM_WORLD, &st);
          ...
          if (len1 > 0)
            MPI_Recv(&seg_results[i].lcs_string[0], len1, MPI_CHAR, owner,
                     static_cast<int>(i) * 10 + 2, MPI_COMM_WORLD, &st);
          MPI_Recv(&len2, 1, MPI_INT, owner, static_cast<int>(i) * 10 + 3,
                   MPI_COMM_WORLD, &st);
          seg_results[i].gapped_seq1.resize(len2);
          if (len2 > 0)
            MPI_Recv(&seg_results[i].gapped_seq1[0], len2, MPI_CHAR, owner,
                     static_cast<int>(i) * 10 + 4, MPI_COMM_WORLD, &st);
          MPI_Recv(&len3, 1, MPI_INT, owner, static_cast<int>(i) * 10 + 5,
                   MPI_COMM_WORLD, &st);
          seg_results[i].gapped_seq2.resize(len3);
          if (len3 > 0)
            MPI_Recv(&seg_results[i].gapped_seq2[0], len3, MPI_CHAR, owner,
                     static_cast<int>(i) * 10 + 6, MPI_COMM_WORLD, &st);
        }
      }
      for (size_t i = 0; i < best_chain.seeds.size(); ++i) {
        final_res.lcs_string += seg_results[i].lcs_string;
        final_res.lcs_length += seg_results[i].lcs_length;
        final_res.gapped_seq1 += seg_results[i].gapped_seq1;
        final_res.gapped_seq2 += seg_results[i].gapped_seq2;
        const auto& anc   = best_chain.seeds[i];
        std::string exact = x_o.substr(anc.query_pos, anc.len);
        final_res.lcs_string += exact;
        final_res.lcs_length += anc.len;
        final_res.gapped_seq1 += exact;
        final_res.gapped_seq2 += exact;
      }
      final_res.lcs_string += seg_results.back().lcs_string;
      final_res.lcs_length += seg_results.back().lcs_length;
      final_res.gapped_seq1 += seg_results.back().gapped_seq1;
      final_res.gapped_seq2 += seg_results.back().gapped_seq2;
    } else {
      for (size_t i = 0; i < segments.size(); ++i) {
        if (static_cast<int>(i % world_size) == world_rank) {
          MPI_Send(&seg_results[i].lcs_length, 1, MPI_INT, 0,
                   static_cast<int>(i) * 10 + 0, MPI_COMM_WORLD);
          int len1 = seg_results[i].lcs_string.size(),
              len2 = seg_results[i].gapped_seq1.size(),
              len3 = seg_results[i].gapped_seq2.size();
          MPI_Send(&len1, 1, MPI_INT, 0, static_cast<int>(i) * 10 + 1,
                   MPI_COMM_WORLD);
          if (len1 > 0)
            MPI_Send(seg_results[i].lcs_string.data(), len1, MPI_CHAR, 0,
                     static_cast<int>(i) * 10 + 2, MPI_COMM_WORLD);
          MPI_Send(&len2, 1, MPI_INT, 0, static_cast<int>(i) * 10 + 3,
                   MPI_COMM_WORLD);
          if (len2 > 0)
            MPI_Send(seg_results[i].gapped_seq1.data(), len2, MPI_CHAR, 0,
                     static_cast<int>(i) * 10 + 4, MPI_COMM_WORLD);
          MPI_Send(&len3, 1, MPI_INT, 0, static_cast<int>(i) * 10 + 5,
                   MPI_COMM_WORLD);
          if (len3 > 0)
            MPI_Send(seg_results[i].gapped_seq2.data(), len3, MPI_CHAR, 0,
                     static_cast<int>(i) * 10 + 6, MPI_COMM_WORLD);
        }
      }
    }
  } else {
    int start_row = 0, local_rows = 0;
    get_row_partition(m, world_size, world_rank, start_row, local_rows);
    std::vector<std::vector<int>> Lblock(local_rows + 1,
                                         std::vector<int>(n + 1, 0));
    if (world_rank > 0)
      MPI_Recv(Lblock[0].data(), n + 1, MPI_INT, world_rank - 1, 300,
               MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    for (int ii = 1; ii <= local_rows; ++ii) {
      ...
      Lblock[ii][0] = 0;
      for (int j = 1; j <= n; ++j) {
        if (x_o[global_i - 1] == y_o[j - 1])
          Lblock[ii][j] = Lblock[ii - 1][j - 1] + 1;
        else
          Lblock[ii][j] = std::max(Lblock[ii - 1][j], Lblock[ii][j - 1]);
      }
    }

    if (world_rank + 1 < world_size)
      MPI_Send(Lblock[local_rows].data(), n + 1, MPI_INT, world_rank + 1, 300,
               MPI_COMM_WORLD);
    std::vector<std::vector<int>> fullL =
        gather_score_matrix_rows(Lblock, m, n, false, MPI_COMM_WORLD);

    if (world_rank == 0) {
      std::vector<std::vector<char>> B;
      final_res = traceback_lcs_from_full(x_o, y_o, fullL, B, lcs_path_coords);
      if (binary)
        writeDPMatrix(fullL, odir_lcs + "/lcs_dp_lengths.bin");
      else if (txt)
        writeRawDPMatrix(fullL, odir_lcs + "/lcs_dp_lengths.txt");
      if (binary)
        writeCharMatrix(B, odir_lcs + "/lcs_traceback_pointers.bin");
      else if (txt)
        writeRawCharMatrix(B, odir_lcs + "/lcs_traceback_pointers.txt");
    }
  }
  ...
  if (world_rank == 0) {
    ...
    std::string acc1 = getAccession(h1_lcs, mode_lcs),
                acc2 = getAccession(h2_lcs, mode_lcs);
    ...
    if (out_aln)
      savePlainAlignment(acc1 + "_LCS_aligned", acc2 + "_LCS_aligned",
                         final_res.gapped_seq1, final_res.gapped_seq2, out_aln);
  ...
  try {
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      if (current_arg == "--query" && i + 1 < argc)
        ...
        if (rank_val == 0)
          std::cerr << "Unknown option: " << current_arg << "\n";
        MPI_Abort(MPI_COMM_WORLD, 1);
        return 1;
      }
    }
    ...
    const FMIndex* fm_target_raw_ptr_val =
        (rank_val == 0) ? fm_index_target_obj_ptr.get() : nullptr;
